...
<body>
    ...
    // Строим SQL
    $sql = "SELECT * FROM psus"; // или другая таблица
    $conditions = [];
    $params = [];
    ...
    <dialog class="add_form">
        <form method="post" action="/lib/handlers/psus_crud.php" enctype="multipart/form-data">
                        ...
                        <div class="accordion">
                            ...
                            <div class="accordion__content">
                                <div class="accordion__content-inner">
                                    ...
                                    <p class="data__item-info"><span>Мощность (Вт):</span>
                                        <?= htmlspecialchars($product['power'] ?? '', ENT_QUOTES) ?></p>
                                    <p class="data__item-info"><span>Форм-фактор:</span>
                                        <?= htmlspecialchars($product['form_factor'] ?? '', ENT_QUOTES) ?></p>
                                    <p class="data__item-info"><span>Основной разъём:</span>
                                        <?= htmlspecialchars($product['main_connector'] ?? '', ENT_QUOTES) ?></p>
                                    <p class="data__item-info"><span>Разъёмы для процессора:</span>
                                        <?= htmlspecialchars($product['cpu_connectors'] ?? '', ENT_QUOTES) ?></p>
                                    <p class="data__item-info"><span>PCIe разъёмы:</span>
                                        <?= htmlspecialchars($product['pcie_connectors'] ?? '', ENT_QUOTES) ?></p>
                                    <div class="data__buttons-group">
                                        <button type="button" class="update-btn" data-id="<?= $product['id'] ?>"
                                            ...
                                            data-main_connector="<?= htmlspecialchars($product['main_connector'] ?? '', ENT_QUOTES) ?>"
                                            data-cpu_connectors="<?= htmlspecialchars($product['cpu_connectors'] ?? '', ENT_QUOTES) ?>"
                                            data-pcie_connectors="<?= htmlspecialchars($product['pcie_connectors'] ?? '', ENT_QUOTES) ?>">Изменить</button>
                                        <form method="post" action="/lib/handlers/psus_crud.php">
